...
class StaticProfile:
    ...
    def auto_populate_from_moodle(self, site_info: dict, courses: list[str]):
        ...
        if university and ("- Üniversite: Bilkent" in profile or "- Üniversite: \n" in profile or "- Üniversite:\n" in profile):
...
class DynamicMemoryDB:
        ...
        with self._conn() as conn:
            ...
            cur = conn.execute(
                """INSERT INTO semantic_memory
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                (entry.category, entry.content, entry.source_message_id,
                 entry.course, entry.confidence, now, now, entry.ttl_days),
            )
            return cur.lastrowid

    def get_memories(self, category: str = None, course: str = None,
                     limit: int = 30) -> list[MemoryEntry]:
    ...
    def get_memories_for_context(self, course: str = None,
                                 max_tokens: int = 800) -> list[MemoryEntry]:
        """
    ...
    def deactivate_memory(self, memory_id: int):
        with self._conn() as conn:
            conn.execute(
                "UPDATE semantic_memory SET is_active = 0 WHERE id = ?", (memory_id,)
            )
    ...
    def update_learning(self, course: str, topic: str, mastery_delta: float = 0.0,
                        asked: bool = False, correct: bool = False, notes: str = ""):
        now = datetime.now(timezone.utc).isoformat()
        with self._conn() as conn:
            existing = conn.execute(
            ...
            if existing:
                ...
                conn.execute(
                    ...
                    (new_mastery, int(asked), int(correct), now,
                     notes, notes, course, topic),
                )
            else:
                conn.execute(
                    """INSERT INTO learning_progress
                       (course, topic, mastery_level, times_asked, times_correct,
                        last_studied, notes)
                       VALUES (?, ?, ?, ?, ?, ?, ?)""",
                    (course, topic, max(0.0, mastery_delta),
                     int(asked), int(correct), now, notes),
                )
    ...
    def get_stats(self) -> dict:
        with self._conn() as conn:
            return {
                ...
                "tracked_topics": conn.execute(
                    "SELECT COUNT(*) c FROM learning_progress"
                ).fetchone()["c"],
            }
...
class HybridMemoryManager:
    ...
    def record_exchange(self, user_message: str, assistant_response: str,
                        course: str = "", rag_sources: str = ""):
        if not self.current_session_id:
            self.start_session(course)

        user_msg_id = self.db.add_message(
            self.current_session_id, "user", user_message, rag_sources,
        )
        self.db.add_message(
