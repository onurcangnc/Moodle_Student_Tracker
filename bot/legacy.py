...
from core.memory import MemoryManager, StaticProfile
...
def _format_completion_message(state: dict) -> str:
    ...
    return f"🎉 <b>{name}</b> tamamlandı! ({total} bölüm okundu)\n\n" "Aşağıdaki butonlarla devam edebilirsin:"
...
def _reading_resume_button() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([[InlineKeyboardButton("▶️ Okumaya Dön", callback_data="rd|resume")]])
...
def _save_file_summaries():
    try:
        ...
        FILE_SUMMARIES_PATH.write_text(json.dumps(file_summaries, ensure_ascii=False, indent=2))
    except Exception as e:
...
def _load_file_summaries():
    ...
    if FILE_SUMMARIES_PATH.exists():
        try:
            file_summaries.update(json.loads(FILE_SUMMARIES_PATH.read_text(encoding="utf-8")))
...
def _sync_blocking() -> dict | None:
    ...
    profile.auto_populate_from_moodle(site_info=moodle.site_info, courses=[c.fullname for c in courses])
...
async def send_long_message(update, text: str, reply_markup=None, parse_mode=None):
    ...
    for i, chunk in enumerate(chunks):
        ...
        try:
            ...
            logger.warning("send_long_message parse mode fallback triggered: %s", exc)
            kwargs.pop("parse_mode", None)
            await send_func(chunk, **kwargs)
        finally:
            elapsed_ms = (time.perf_counter() - send_start) * 1000.0
            logger.debug("Telegram message chunk sent in %.2f ms (size=%s)", elapsed_ms, len(chunk))
...
def courses_keyboard():
    try:
        ...
        logger.warning("Courses keyboard fallback used: %s", exc)
...
async def _generate_summary_msg(update: Update, course_query: str):
    ...
    for c in courses:
        ...
        if (
            query_lower in c.fullname.lower()
            or query_lower in sn
            or query_lower.replace(" ", "") in sn.replace(" ", "")
        ):
            match = c
            break
...
def _format_assignments() -> str:
    ...
    lines.append("\n───────────────")
    lines.append(
        f"📊 Toplam: {len(assignments)} | Bekleyen: {len(pending)} | Geçmiş: {len(overdue)} | Teslim: {len(submitted)}"
    )
...
async def cmd_login(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result["status"] == "sms_sent":
        ...
        if webmail_client.authenticated:
            ...
            if code:
                await msg.edit_text("🔑 Kod bulundu, doğrulanıyor...")
...
def _inject_assignments_context():
    ...
    try:
        ...
        logger.warning("Assignments context injection failed: %s", exc)
...
async def cmd_mail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not webmail_client.authenticated:
        await update.message.reply_text(
            "📬 Webmail bağlantısı yok. .env'de WEBMAIL_EMAIL ve WEBMAIL_PASSWORD tanımlayın."
        )
        return
    ...
    if not mails:
        ...
        try:
            ...
            logger.warning("Recent mail fallback failed: %s", exc)
            mails = []
        if not mails:
    ...
    try:
        ...
        summary_lines = [line.strip() for line in summaries_raw.strip().split("\n") if line.strip()]
    except Exception as e:
    ...
    try:
        ...
        for mail in new_mails:
            ...
            try:
                ...
                logger.warning("Mail background summary fallback used: %s", exc)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if match:
                ...
                while remaining:
                    ...
                    try:
                        ...
                        logger.warning("Summary chunk parse mode fallback used: %s", exc)
    ...
    if data.startswith("upassign_"):
        ...
        try:
            count = await asyncio.to_thread(_index_uploaded_file_sync, Path(upload_path), match.fullname, upload_name)
            await query.edit_message_text(
    ...
    if data.startswith("rf|"):
        ...
        mode_notice = (
            "📖 <i>Okuma moduna geçildi. Normal moda dönmek için</i> [💬 Normal Mod] <i>butonunu kullan.</i>\n\n"
        )
    ...
    if data.startswith("cs|"):
        ...
        if not course_files:
            ...
            await query.edit_message_text(
                ...
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Ders Listesi", callback_data="cs|back")]]),
            )
            return
    ...
    if data.startswith("srs|"):
        ...
        if srs_action == "grades_detail":
            ...
            await query.edit_message_text(
                "\n".join(lines),
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Geri", callback_data="srs|back")]]),
            )
            return
    ...
    if data.startswith("rd|"):
        ...
        if action == "normal":
            ...
            total = state["reading_total"]
            keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("📖 Okumaya Dön", callback_data="rd|resume")]])
            await query.edit_message_text(
...
def _detect_course(filename: str) -> str | None:
    try:
        ...
        logger.warning("Course detection course fetch failed: %s", exc)
        return None
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if detected:
        ...
        try:
            ...
            logger.error("Failed to load course list for upload assignment: %s", exc)
    ...
    if cache.schedule:
        ...
        for target_day in [today_name, tomorrow_name]:
            ...
            if entries:
                ...
                day_items = ", ".join(f"{e.get('course', '?')} {e.get('time', '')}" for e in entries)
...
def _get_assignments_context_string() -> str:
    ...
    try:
        ...
        logger.warning("Assignment context fetch failed: %s", exc)
        return ""
...
async def _handle_sync_keyword(update: Update, uid: int, user_msg: str):
    ...
    stats = vector_store.get_stats()
    msg = await update.message.reply_text(f"🔄 Sync başlıyor...\n📦 Mevcut: {stats.get('total_chunks', 0)} chunk")
...
async def _handle_mail_keyword(update: Update, uid: int, user_msg: str):
    ...
    try:
        ...
        if not mails:
            mails = await asyncio.wait_for(
                asyncio.to_thread(webmail_client.get_recent_airs_dais, 5),
                timeout=45,
            )
            is_unread = False
...
async def _handle_homework(update: Update, uid: int, state: dict):
    ...
    text = _format_assignments()
    reply_markup = (
        _reading_resume_button() if (state.get("reading_mode") or state.get("reading_paused")) else back_keyboard()
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if _is_continue_command(user_msg):
                ...
                if not batch:
                    ...
                    buttons = _reading_buttons(state, completed=True)
                    await send_long_message(
                        update, f"{msg_text}{footer}", parse_mode=ParseMode.HTML, reply_markup=buttons
                    )
                    return
            ...
            await send_long_message(
                update, f"📚 {response}{footer}", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([nav])
            )
            save_to_history(uid, user_msg, response, active_course=course)
            return
    ...
    try:
        ...
        if is_continue and not results:
            typing.stop()
            await update.message.reply_text(
                "Bu konuda elimdeki materyallerin sonuna geldik. " "Başka bir konuya geçmek ister misin?"
            )
        ...
        if course_filter:
            ...
            if not course_has_materials:
                display_response = (
                    f"ℹ️ <b>{course_filter}</b> dersinin Moodle'da materyali yok. " "Genel bilgimle yanıtlıyorum:\n\n"
                ) + display_response
...
async def _run_post_sync_eval(bot=None):
    ...
    try:
        from tests.test_rag_quality import TEST_QUERIES, eval_all, generate_auto_queries, save_baseline
            ...
            if delta < -0.05:
                ...
                if bot and OWNER_ID:
                    try:
                        ...
                        logger.error("Failed to send RAG regression notification: %s", exc)
            elif delta > 0.05:
    ...
    async with sync_lock:
        try:
            ...
            if new_chunks > 0:
                ...
                if OWNER_ID:
                    try:
                        ...
                        logger.error("Failed to send auto-sync notification: %s", exc)
            else:
            ...
            try:
                ...
                logger.warning("Context refresh after auto-sync failed: %s", exc)
...
async def auto_stars_login_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if result["status"] == "sms_sent":
            ...
            logger.info("Auto STARS login: code found, verifying...")
            verify_result = await asyncio.to_thread(stars_client.verify_sms, uid, code)
            if verify_result["status"] != "ok":
        ...
        if cache:
            ...
            if now - last_stars_notification >= STARS_NOTIFY_INTERVAL:
                ...
                try:
                    # Build summary
                    info = cache.user_info or {}
                    lines = ["📊 <b>STARS Güncellendi</b> (otomatik)"]
                    if info.get("cgpa"):
                    ...
                    if cache.exams:
                        ...
                        for ex in cache.exams[:3]:
                            lines.append(
                                f"  • {ex.get('course', '?')}: {ex.get('exam_name', '?')} ({ex.get('date', '?')})"
                            )
                    if cache.attendance:
                        att_summary = ", ".join(
                            f"{a.get('course', '?').split()[0]}: {a.get('ratio', '?')}" for a in cache.attendance[:5]
                        )
                    ...
                    logger.error("Failed to send STARS summary notification: %s", exc)
        else:
...
def _build_stars_snapshot(cache) -> dict:
    ...
    return {
        "exams": {(e.get("course", ""), e.get("exam_name", ""), e.get("date", "")) for e in (cache.exams or [])},
        "grades": {
            (g.get("course", ""), a.get("name", ""), a.get("grade", ""))
            for g in (cache.grades or [])
            for a in g.get("assessments", [])
        },
        "attendance": {(a.get("course", ""), a.get("ratio", "")) for a in (cache.attendance or [])},
    }
...
async def deadline_reminder_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        assignments = moodle.get_assignments()
        urgent = [a for a in assignments if not a.submitted and 0 < a.due_date <= three_days and a.due_date > now]
        ...
        if urgent and OWNER_ID:
            ...
            try:
                ...
                logger.error("Failed to send deadline reminder notification: %s", exc)
...
async def post_init(app: Application):
    ...
    app.job_queue.run_repeating(
        assignment_check_job, interval=ASSIGNMENT_CHECK_INTERVAL, first=60, name="assignment_check"
    )
        ...
        try:
            ...
            if result["status"] == "sms_sent" and webmail_client.authenticated:
                ...
                if cache:
                    ...
                    llm.invalidate_student_context()
                    await _notify_stars_changes(cache, app)
                    logger.info("Auto STARS login at startup: OK (no 2FA)")
    ...
    logger.info(
        f"Auto-sync: every {AUTO_SYNC_INTERVAL // 60} min | Assignment check: every {ASSIGNMENT_CHECK_INTERVAL // 60} min"
    )
    logger.info("STARS auto-login: every 10 min | Mail check: every 30 min")
...
def main():
    ...
    if not token:
        ...
        logger.error("Telegram -> @BotFather -> /newbot")
        logger.error(".env'e ekle: TELEGRAM_BOT_TOKEN=xxx")
        sys.exit(1)

    if not OWNER_ID:
        logger.error("TELEGRAM_OWNER_ID not set - guvenlik riski.")
        logger.error(".env'e ekle: TELEGRAM_OWNER_ID=senin_chat_id")
        logger.error("Chat ID ogrenmek icin: Telegram -> @userinfobot")
        sys.exit(1)

    logger.info("Bilesenler yukleniyor...")
    init_components()
    ...
    logger.info("Bot calisiyor (owner=%s)", OWNER_ID or "herkes")
    logger.info(
        "Auto-sync: her %s dk | Odev check: her %s dk",
        AUTO_SYNC_INTERVAL // 60,
        ASSIGNMENT_CHECK_INTERVAL // 60,
    )
    logger.info("Mail: her 30 dk | STARS auto-login: her 10 dk")
